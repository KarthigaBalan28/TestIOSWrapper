...
@property (strong) id<HIDApproveCallback> pwdCallback;
@property (strong) id<HIDApproveCallback> exceptionCallback;
@end
...
*
* @returns an instance of ContainerEventListener
*
*/
- (id)initWithParams:(id<HIDApproveCallback>) pwdCallback withExceptionCallback :(id<HIDApproveCallback>) exceptionCallback
{
    self = [super init];
...
* @returns an instance of HIDEventResult with the appropriate code and password if applicable
*/
...
-(void)createContainer:(NSString *)activationCode withPushId:(NSString *)PushId withPwdCallBack:(id<HIDApproveCallback>)pwdCallback withExCallback:(id<HIDApproveCallback>)ExceptionCallback{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0),^{
        NSLog(@"AppproveSDKWrapper ---> HID:createContainer createContainer is called");
        NSError* deviceError;
        ...
        if(deviceError != nil){
            ...
            NSLog(@"ApproveSDKWrapper ---> HID:createContainer  Creation Complete");
...
*/
-(void) executeGenericCallback : (id<HIDApproveCallback>)callback withParams : (NSArray *)params{
...
-(void) renewContainer:(NSString *)password withPwdCallBack:(id<HIDApproveCallback>)promptCallback withExceptionCallBack:(id<HIDApproveCallback>)ExceptionCallback{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0),^{NSLog(@"AppproveSDKWrapper ---> HID:renewContainer renewContainer is called");
...
//        [config setContainerFriendlyName:@"HID"];
        ...
        [currentContainer renew:config withSessionPassword:nil withListener:renewListener error:&renewalError];
        if(renewalError != nil){
...
*/
-(int) getContainerRenewableData:(id<HIDContainer>)container callback:(id<HIDApproveCallback>)genericExecuteCallback{
    NSError* error;
...
-(int)getContainerRenewableDate {
    ...
    NSLog(@"ApproveSDKWrapper ---> HID:getContainerRenewableDate HID In getContainerRenewableDate");
    NSError *error = nil;
    NSDate *expiryDate = [pContainer getExpiryDate:&error];
    if (error) {
        // Handle the error, e.g., log it, show a message, or take appropriate action.
        NSLog(@"ApproveSDKWrapper ---> HID:getContainerRenewableDate Error getting expiry date: %@", [error localizedDescription]);
        return (pow(10, 9) + 7);
    }
    ...
    if (error) {
        // Handle the error, e.g., log it, show a message, or take appropriate action.
        NSLog(@"ApproveSDKWrapper ---> HID:getContainerRenewableDate Error getting creation date: %@", [error localizedDescription]);
        return (pow(10, 9) + 7);
    }
    
    NSTimeInterval containerExpiry = [expiryDate timeIntervalSince1970];
    NSLog(@"ApproveSDKWrapper ---> HID:getContainerRenewableDate Container Expiry Date is %f", containerExpiry);
    NSTimeInterval containerStart = [creationDate timeIntervalSince1970];
    NSLog(@"ApproveSDKWrapper ---> HID:getContainerRenewableDate Container Creation Date is %f", containerStart);
    ...
    NSLog(@"ApproveSDKWrapper ---> getContainerRenewableDate -- startDays: %d",startDays);
    NSLog(@"ApproveSDKWrapper ---> getContainerRenewableDate -- endDays: %d",endDays);
...
-(int)calFinalDays:(int)total end:(int)end {
    NSLog(@"ApproveSDKWrapper ---> HID:getContainerRenewableDate  calFinalDays Total Days is %d", total);
    NSLog(@"ApproveSDKWrapper ---> HID:getContainerRenewableDate  calFinalDays End Days is %d", end);
    float perc = ((float)end / (float)total) * 100;
    NSLog(@"ApproveSDKWrapper ---> HID:getContainerRenewableDate  calFinalDays Percentage is %f", perc);
    if (end<=2 || perc < 20.0f) return end;  // if 2 or fewer days are left || percentage is in last 20% of expiry time
    NSLog(@"ApproveSDKWrapper ---> HID:getContainerRenewableDate  calFinalDays Returning %d", (-1 * end));
    return (-1 * end);
}
...
- (long)getWrtCurrentTime:(long)time {
    ...
    NSLog(@"ApproveSDKWrapper ---> HID:getWrtCurrentTime getContainerRenewableDate -- currentMilli: %ld",currentMilli);
    NSLog(@"ApproveSDKWrapper ---> HID:getWrtCurrentTime getContainerRenewableDate -- currentMilli/1000: %ld",currentMilli/1000);
...
*
* @param pushId - The Push ID for the container.
...
-(NSString *)getLoginFlow:(NSString *)pushId callBack:(id<HIDApproveCallback>)genericExecutionCallback{
    NSLog(@"ApproveSDKWrapper ---> HID:getLoginFlow  New called from Wrapper Framework with new change");
...
*/
-(void)generateOTPInternal:(NSString *)password isBioEnabled:(bool)bioEnabled withSuccessCB:(id<HIDApproveCallback>)success_CB failureCB:(id<HIDApproveCallback>)failure_CB otpKeyLabel : (NSString *) otpKeyLabel{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0),^{
        NSError* error;
        id<HIDContainer> pContainer = [self getSingleUserContainer];
        if(pContainer == nil){
...
//        [filter addObject:[HIDParameter parameterWithString:otpKeyLabel forKey:HID_KEY_PROPERTY_LABEL]];
        ...
        if (!pKey) {
            ...
            [self executeGenericCallback:(failure_CB) withParams:(@[@"OTPException",@"No OTP key found"])];
        }else{
            if (keys.count > 1) {
        ...
        if(error != nil){
            ...
            if(errorCode == 100){
                ...
                NSLog(@"ApproveSDKWrapper ---> HID:generateOTP Biometric cancled or wrong fingerprint entered");
...
*/
-(void)signTransactionInternal:(NSString *)transactionDetails withPwdPromptCallback:(id<HIDApproveCallback>)pwdPromptCallback withSuccessCB:(id<HIDApproveCallback>)successCB withFailureCB:(id<HIDApproveCallback>)failureCB withOTPKey:(NSString *) otp_Key{ dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0),^{
    NSError *error;
    NSLog(@"ApproveSDKWrapper ---> HID:signTransaction InsideSignTransaction");
    bool isBioEnabled = [self checkBioAvailability];
    id<HIDContainer> pContainer = [self getSingleUserContainer];
    NSMutableArray* filter = [[NSMutableArray alloc] init];
//    [filter addObject:[HIDParameter parameterWithString:@"OATH_OCRA_event_SIGN" forKey:HID_KEY_PROPERTY_LABEL]];
    [filter addObject:[HIDParameter parameterWithString:HID_KEY_PROPERTY_USAGE_OTP forKey:HID_KEY_PROPERTY_USAGE]];
    ...
    if (!pKey) {
        ...
        [self executeGenericCallback:(failureCB) withParams:(@[@"OTPException",@"No OTP key found"])];
    }else{
        if (keys.count > 1) {
...
*/
-(NSString* )getPasswordFromUIAsync : (NSString* ) eventType withEventCode:(NSString*) eventCode withPwdPromptCB: (id<HIDApproveCallback>) pwdPromptCB{
...
* @param pwdPromptCB - Callback function to prompt for password.
*/
-(void)invokeTsPasswordAuth : (id<HIDAsyncOTPGenerator>) pAsyncOTPGenerator withChallenge:(NSString*) challenge withEventType:(NSString*)eventType  withEventCode:(NSString*) eventCode withSuccessCB: (id<HIDApproveCallback>)successCB withFailureCB: (id<HIDApproveCallback>)failureCB withPwdPromptCB:(id<HIDApproveCallback>) pwdPromptCB{
    NSError *error;
...
*/
-(NSString *) retreiveTransaction:(NSString *)txID withPassword:(NSString *)pwd isBioEnabled:(bool)isBioEnabled withCallback:(id<HIDApproveCallback>)callback{
    NSError* deviceError;
    NSError* error;
    HIDConnectionConfiguration* connectionConfig = [[HIDConnectionConfiguration alloc] init];
    id<HIDDevice> pDevice = [[HIDDeviceFactory alloc] getDevice:connectionConfig error:&deviceError];
...
*/
-(void) setNotificationStatus:(NSString *)txID withStatus:(NSString *)status withPassword:(id)pwd withJSCallback:(id<HIDApproveCallback>)onCompleteCB withPwdPromptCB:(id<HIDApproveCallback>)pwdPromptCB{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0),^{
...
*/
-(void)invokePasswordAuthNotification: (id<HIDTransaction>) transaction withPassword : (NSString*)pwd withStatus : (NSString* ) status withCompletionCB :(id<HIDApproveCallback>)onCompleteCB withPwdPromptCB : (id<HIDApproveCallback>)promptCB {
    NSError* error;
...
- (void)setContainerFriendlyName:(NSString *)username withFriendlyName: (NSString *)friendlyName withSetNameCallback : (id<HIDApproveCallback>)setNameCallback{
    ...
    if (deviceError != nil) {
        NSLog(@"ApproveSDKWrapper ---> HID:setContainerFriendlyName error while creating container %@", [deviceError userInfo]);
...
- (void)directClientSignatureWithStatus:(NSString *)consensus
                  ...
                  withBiometricEnabled:(BOOL)isBiometricEnabled
                       withDCSCallback:(id<HIDApproveCallback>)dcsCallback {
...
- (void)sendTransactionStatusResult:(id<HIDTransaction>)transaction
                              result:(BOOL)result
                        dcsCallback:(id<HIDApproveCallback>)dcsCallback {
...
*/
- (void)handleSetStatusError:(NSError *)error callback:(id<HIDApproveCallback>)callback {
    int errorCode = (int)[error code];
